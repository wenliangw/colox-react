...
- **`@colox/react`**（`packages/components/`）：组件库本体，组件按目录组织；构建按组件切入口（`index`/`button`/`checkbox`/`container`/`grid`/`icon-button`/`input`/`radio`/`select`/`stack`/`textarea` 多 entry + `exports` 子路径 `@colox/react/button` 等）做 JS 级树摇；`@import '@colox/theme/index.css'` 级联进单一 `style.css`（`cssCodeSplit: false`），保持一行引入。内部 cdk 层（`src/cdk/`，alias `@colox/cdk/*`，不进公共面）：floating 弹层基建（定位包 @floating-ui/dom、portal Popup、dismiss/combox-keyboard，按功能文件夹组织 popup/ + hooks/）+ input-control 裸控件单元（colox-input-control reset，Input/Select 共享）。详见 [modules/button.md](wiki/modules/button.md)、[modules/input.md](wiki/modules/input.md)、[modules/textarea.md](wiki/modules/textarea.md)、[modules/checkbox.md](wiki/modules/checkbox.md)、[modules/radio.md](wiki/modules/radio.md)、[modules/select.md](wiki/modules/select.md)、[modules/stack.md](wiki/modules/stack.md)、[modules/container.md](wiki/modules/container.md)、[modules/grid.md](wiki/modules/grid.md)、[modules/icon-button.md](wiki/modules/icon-button.md)
