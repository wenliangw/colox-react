...
- **`@colox/react`**（`packages/components/`）：组件库本体，组件按目录组织；构建按组件切入口（`index`/`button`/`checkbox`/`container`/`grid`/`input`/`radio`/`stack` 多 entry + `exports` 子路径 `@colox/react/button` 等）做 JS 级树摇；`@import '@colox/theme/index.css'` 级联进单一 `style.css`（`cssCodeSplit: false`），保持一行引入。详见 [modules/button.md](wiki/modules/button.md)、[modules/input.md](wiki/modules/input.md)、[modules/checkbox.md](wiki/modules/checkbox.md)、[modules/radio.md](wiki/modules/radio.md)、[modules/stack.md](wiki/modules/stack.md)、[modules/container.md](wiki/modules/container.md)、[modules/grid.md](wiki/modules/grid.md)
