...
- **`@colox/react`**（`packages/components/`）：组件库本体，组件按目录组织；构建按组件切入口（`index`/`autocomplete`/`button`/`checkbox`/`container`/`date-picker`/`grid`/`icon-button`/`input`/`input-number`/`radio`/`select`/`slider`/`stack`/`switch`/`textarea` 多 entry + `exports` 子路径 `@colox/react/button` 等）做 JS 级树摇；`@import '@colox/theme/index.css'` 级联进单一 `style.css`（`cssCodeSplit: false`），保持一行引入。内部 cdk 层（`src/cdk/`，alias `@colox/cdk/*`，不进公共面）：floating 纯弹层基建（定位包 @floating-ui/dom、portal Popup、dismiss，按功能文件夹组织 popup/ + hooks/）+ combobox 建议行为完整内核（词形 `ComboboxOption` + 筛选纯函数 + `useComboboxKeyboard` 键盘巡行 + `walkComboboxLeaves` 通用走查，Select search 与 AutoComplete 共享）+ input-control 裸控件单元（colox-input-control reset，Input/Select 共享）。详见 [modules/autocomplete.md](wiki/modules/autocomplete.md)、[modules/button.md](wiki/modules/button.md)、[modules/input.md](wiki/modules/input.md)、[modules/input-number.md](wiki/modules/input-number.md)、[modules/date-picker.md](wiki/modules/date-picker.md)、[modules/textarea.md](wiki/modules/textarea.md)、[modules/checkbox.md](wiki/modules/checkbox.md)、[modules/radio.md](wiki/modules/radio.md)、[modules/select.md](wiki/modules/select.md)、[modules/slider.md](wiki/modules/slider.md)、[modules/stack.md](wiki/modules/stack.md)、[modules/switch.md](wiki/modules/switch.md)、[modules/container.md](wiki/modules/container.md)、[modules/grid.md](wiki/modules/grid.md)、[modules/icon-button.md](wiki/modules/icon-button.md)
- **`@colox/icons`**（`packages/icons/`）：第一方基础图标包——stroke 风格（24 viewBox / 1.5 round stroke / currentColor / 1em 默认、`size` 数字 px 覆盖），`IconXxx` 前缀命名的 React 组件（前缀防碰撞，per-icon 命名导出保树摇），`IconBase` 为内部契约基座（不进公共面）；图标设计规范八条由 spec lint（test/icon-spec.test.tsx）机器门禁：几何锁（渲染必须等于设计的 d）、整数网格、[2,22] 光学内容框、成对同源（chevron 四向单几何旋转、eye-off 由 eye 派生）。零运行时依赖。批次一：chevron×4、x、check、plus、eye、eye-off、search、calendar 十一枚样板。详见 [modules/icons.md](wiki/modules/icons.md)
