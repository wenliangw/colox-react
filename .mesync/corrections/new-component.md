...
18. **cdk 能力桶与组件同构**：在 `src/cdk/` 下新建能力桶（combobox 这类）时，目录与类型规范照组件走——能力一律文件夹化（根只留入口 `index.ts`，`types/`/`hooks/`/`filter/` 等能力目录平行分层，**无同级平铺文件**，单功能目录用 `index.ts` 入口）、类型契约集中 `types/` 按层分文件（hook 契约在 `types/hooks.ts`、实现文件**零内联类型**）、JSDoc 全多行。combobox 前科：收齐欠账时照抄 floating 旧形态（filter.ts/walk.ts/types.ts 平铺 + hook 内联 Options/Result 接口 + 单行 JSDoc），被用户指正「目录结构不符合品味规范」后重排——「存量 cdk 也这么写」不豁免，以 code-style.md 75-84 行 + 33-40 行为准。
