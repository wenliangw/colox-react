...
- 设计语言单一来源已落地：Figma variables 导出四文件（color / semantic-color / typography / size）→ `figma-to-tokens.mjs` 转换 → Style Dictionary v4 → `themes/light.css`（133 变量）+ `themes/dark.css`（40 个 delta 覆盖）。语义契约定版：强度四档（solid/muted/subtle/inverse）+ 角色组（text/bg/border），详见 tastes/styling。fontFamily/shadow/motion/breakpoint 为工程侧人工维护（`base.tokens.json`）。**dark 首版已落地**：与主题无关的 palette 不动，仅语义层在 `semantic-color.dark.tokens.json` 重映射（工程侧手维护，Figma 后续出 dark modes 时由导出接管）——推导约定见 tastes/styling。
