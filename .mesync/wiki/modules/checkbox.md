...
├── hooks/
│   ├── use-checkbox-group.ts           # Group 选中态：value/defaultValue 对称 + toggleValue 翻转命令（hook 命令；context 侧以 onChange 事件槽下发，发布 { event, value } 载荷）
...
成员 = 声明 `value` 且未显式 `checked`/`defaultChecked` 的 checkbox：checked 派生自 `group.value.includes(value)`，翻转走组 context 的 `onChange` 事件槽（成员调 `group.onChange(value, event)`，背后是 hook 的 `toggleValue` 命令）；显式控制者独立（`value` 只喂原生表单）。`size`/`name`/`disabled`/`invalid`/`readOnly` 组继承（`size`/`name`/`invalid` 本人优先；`disabled`/`readOnly` 是能力限制、**sticky 不可退出**）。成员受控于组：成员自己的 `onChange` 也发布家族载荷 `CheckboxChangePayload = { event, value }`（value = 该成员的下一勾选态 boolean），与组载荷同形——`event.target.checked` 是 React 受控输入的标准语义（恢复后的受控值）；**下一选中数组从 `Checkbox.Group` 的 `onChange` 读取**（组载荷 `{ event, value }`：event = 触发成员原生合成事件，value = 下一数组）。toggle 纯函数逻辑（含/不含 → 增/删）住 hook，不依赖 DOM 事件目标。

### 受控/非受控对称

叶子：`checked`/`defaultChecked` 原生对称直通（非受控时 `event.target.checked` 即翻转值），`onChange` 发布 `{ event, value: boolean }`（prop `value` 仍是表单 token 字符串，两者不冲突）。Group：`value`/`defaultValue` 对称、统一走组 `onChange({ event, value })` 单一事件流（`useCheckboxGroup` 内 `useState(defaultValue ?? [])` + 受控透传；hook 组装 `{ event, value }` 载荷发布）。
...
- **readOnly（家族自造）**：单件与组级同词——`handleChange` 先判（`!checked` 回滚，`indeterminate` 一并复位）、`aria-readonly`、根类 `colox-checkbox--readonly` + `cursor: default`、不灰化；组级经 context 下发（`readOnly || group.readOnly`）。
- `CheckboxGroupProps extends Omit<HTMLAttributes<HTMLDivElement>, 'onChange' | 'defaultValue'>`，新增：`value?: string[]`、`defaultValue?: string[]`、`onChange?: (payload: CheckboxGroupChangePayload) => void`（payload 带触发成员原生事件 + 下一数组）、`invalid?: boolean`（组级非法：未自设 invalid 的成员一律转红 + `aria-invalid`——**组根不加 aria-invalid**，ARIA 的 group 角色不支持；Form 前补强轮新增）；叶子 `onChange?: (payload: CheckboxChangePayload) => void`（`CheckboxProps` 因此 `Omit<'size'|'type'|'value'|'onChange'>`）、`size?: CheckboxSize`（成员继承、本人优先、缺省 md）、`disabled?`、`name?`。Group 根 div `role="group"` + `colox-checkbox-group`（纵向布局，gap spacing-2）。
