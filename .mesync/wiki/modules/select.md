...
- 清除按钮 Select 自建（`colox-select__clear`），复用 Input 的 keepFocus 模式（mousedown preventDefault + click 清除）。按钮本体与 tag-remove 均为 IconButton（`size="4"`）——复位/方形足迹/聚焦环/禁用态上提基座，站点类只留换位 reveal 与 chip 内着色（决策 07ea9169）；视觉轴落地后（05b83bed → a42ba861 → 644ffa12 → 666d8aa8 → intent→palette 对齐）两钮随默认 text+gray、图标 16px（size="4" 直驱）：tag-remove 静止 muted/hover solid 全为站点色规则胜出不变；clear 静止随基座 gray 灰、hover 由站点 text-solid 规则胜出。
