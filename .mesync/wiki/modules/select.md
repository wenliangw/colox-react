...
- 清除按钮 Select 自建（`colox-select__clear`），复用 Input 的 keepFocus 模式（mousedown preventDefault + click 清除）。按钮本体与 tag-remove 均为 IconButton（`size="4"`）——复位/方形足迹/聚焦环/禁用态上提基座，站点类只留换位 reveal 与 chip 内着色（决策 07ea9169）；视觉轴落地后（05b83bed）两钮随默认 ghost+primary 获得方形浅底 hover wash，与站点 hover 色规则叠加。
