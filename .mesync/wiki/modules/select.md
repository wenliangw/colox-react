...
- **showSearch**（默认 false）：控制位形态开关——**开启** = 内嵌输入 control（single 与 multiple 同形；single 关闭时显示选中项 text、打开时翻转到 query 流，multiple 输入兼 chip 行），过滤 = text + value 大小写不敏感子串，`filterOption` 可替换，`onSearch(query)` 原始流直出（远程编排归消费方）；**关闭** = select-only 触发按钮（single 与 multiple 同形）——只能聚焦/开面板/选择，**永不接受输入**（multiple 无 showSearch 不是可编辑控件；用户 2026 指正：「没有设置 showSearch 不应该可以输入只能选择」）。
- **面板开合的三个通道（用户两轮指正后定案）**：① **click toggle**——触发按钮点开/点关（恢复原行为）；**指针/键盘分流**消竞态：mousedown 先打标记 → 指针引发的 focus 不开（交给 click 定夺），Tab 等键盘聚焦无标记 → 直接开；**input 形态的 click 只开不关**（点击是放光标，antd 同款）。② **focus 开**（开在初始高亮=选中项/首位）、**blur 收**并复位巡行。③ **window blur 收**（走 cdk `useDismissible` 通道）：iframe 预览/切窗口时元素层 blur **不触发**、只有 window blur 会来——缺这条就出现「组件 blur 了面板却不收」（用户 storybook 实测）。其余通道：Escape 关、外部 pointerdown 关；行 + 面板 padding + chip 移除 + clear 均 mousedown preventDefault 保焦点（行点击不失焦 → 面板不误收）。打开后上下方向键循环走（跳过 disabled），Home/End 跳首尾，Enter 激活；Space 只在非 input control 上拦截（`event.currentTarget instanceof HTMLInputElement` 判别）。
...
- **ref**：`SelectRef = HTMLInputElement | HTMLButtonElement` 联合——暴露焦点 control（普通 single 与无 showSearch 的 multiple = 触发按钮；可搜索 single/multiple = 内嵌 input）。
...
100 例全绿（v1 的 61 例全部保留并改写为新契约）：键盘循环/跳 disabled/Home/End、Enter 载荷（event.key 断言）、受控对称、mouse 选择载荷（记录 + event.target）、重新选择同值仍发射、clearable（payload option undefined）、隐藏 input、invalid/disabled、过滤（默认/自定义/onSearch）、富 children 渲染 + text 行名钉住、size 继承/逐行覆盖、传透包装编译、受控 open、默认 open（SSR mounted 守卫）、外部点击/Escape 关闭、**click toggle（按钮二次点击关）+ 指针先聚焦时 click 定夺（mousedown→focus→click 序列）+ input 形态 click 只开不关 + focus 开 / blur 收（single button、single searchable、multiple button、multiple searchable 四形态 + onOpenChange 载荷）+ window blur 收（iframe/切窗口通道）**、multiple chip/Backspace/Enter 反选/多隐藏 input、focus 保持、空态、**无 showSearch 的 multiple = 触发按钮 + shell 内零文本 input（select-only 断言）**、ref 双形态（可搜索 = input / 不可搜索 = button）、折叠计数纯函数 4 例 + jsdom 徽标隐藏 1 例 + 模拟指标回归 1 例（inner 预算折叠与放宽长回双向断言，防「折叠自食」归零）、tag 模板 9 例（注入契约/包装发现/未声明值合成兜底/onRemove 载荷+面板不弹/折叠隐藏包注入自定义根/宿主元素/无子/重复模板/未知槽硬错误）+ tag 模板受控移除 2 例（面板关/开两态 pointerdown+click 序列）。测试依赖 test-setup.ts 的 ResizeObserver stub（autoUpdate + 折叠重测需要）；option 的 id 用 `document.getElementById` 查询（useId 冒号不能进选择器）。
...
- 2026-10 muted 档换装（决策 49d74585）：四站统一 variant="muted"（静止 text-muted→hover/active text-default 语境音色），tag-remove/clear 两条站点色规删除，站点层纯结构；探针 MU/Field 逐字节实证。
...
- 2026-10 Select 行为修复两刀（用户报 bug）：① **无 showSearch 的 multiple 改为 select-only 触发按钮**（形态选择器从 `isMultiple || showSearch` 收敛为 `showSearch`）——此前 multiple 恒渲染可编辑 input，打字虽不筛（过滤受 showSearch 门控）却能输入，语义错位；② **面板改为 focus/blur 驱动**——focus 开（鼠标/Tab 一致）、blur 收（点页面他处即收），click 从 toggle 改纯开（消除 focus+click 竞态：mousedown 聚焦已开、click 再 toggle 会立刻关掉）；配套保焦点：行 + 面板 padding + chip 移除钮 + clear 钮 mousedown preventDefault（行点击允许 pick 而面板不误收）。`resolveButtonDisplay` 新增 `isMultiple`/`hasMultipleValues`（multiple 空态按钮显示 placeholder、有 chip 时按钮留空）。94 例全绿。
- 2026-10 Select 开合模型二轮修正（用户指正上一轮矫枉过正「仍然需要保留 click 的 toggle 能力」「上一次我的语言有些绝对了」）：① **click toggle 恢复**（按钮形态点开/点关），配 **指针/键盘分流**消竞态——mousedown 先打标记，指针引发的 focus 不开（click 定夺 toggle），Tab 聚焦无标记直接开；**input 形态 click 只开不关**（放光标，antd 同款）② **iframe/窗口失焦收面板**——用户 storybook 实测「点了目录 Select blur 了面板却不收」，根因：iframe 预览里点击宿主页面时只有 window blur、**元素层 blur 不触发**，`onBlur` 从未运行；修在 cdk `useDismissible` 增 window blur 通道（Select/AutoComplete/DatePicker 共有的洞，一处修三处受益）。100 例全绿。
- 2026-10 Select 形态切换的布局缺陷（真实浏览器探针抓出）：multiple 无 showSearch 换成触发按钮后，有 chip 时按钮无内容 → **高度塌成 0**（76×0，不可见/不可点，Playwright 判 hidden；jsdom 无布局全绿放行）；修 `.colox-select__control { align-self: stretch }`（由行高撑起确定盒）。配套真实浏览器探针 12 项全过：select-only 无输入、click toggle 开/关、focus 开、**离开 iframe（点 storybook 目录等效）收面板**、点页面背景收面板、可搜索多选过滤、input 点击保持开、单选选中关。纠错条款 corrections/hit-testing.md（空交互件确定盒）。
- 2026-10 Select 焦点环修复（用户报「频繁 Tab 时部分组件有黑边」）：触发钮（button 形态）漏抑制引擎 UA 默认环（`outline: auto 1px rgb(16,16,16)` ≈ 黑边），与外壳 `:focus-within` 的 brand 环叠加——家族其它控件（input-control/checkbox/radio/switch/slider/button/input-number）皆已抑制，Select 按钮形态是唯一漏网 → `.colox-select__control { outline: none }`，焦点指示唯一归属外壳环。真实浏览器实测：触发钮 outline none + 外壳环 `#4F46E5` + 光晕。（Select 内 clear/chip 移除/模板图标钮的环来自 IconButton 基座，仍保持跟随 palette 的中性灰——品牌环方案被用户否决，见 icon-button 模块。）纠错条款 corrections/focus-indicator.md。
