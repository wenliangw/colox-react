...
叶子：`checked`/`defaultChecked` 原生对称直通（非受控时 `event.target.checked` 即翻转值；受控 radio 只能在「选中未选中者」时触发 change），`onChange` 发布 `{ event, value: boolean }`（prop `value` 仍是组键字符串）。Group：`value`/`defaultValue` 对称、统一走组 `onChange({ event, value })` 单一事件流（`useRadioGroup` 内 `useState(defaultValue ?? '')` + 受控透传；hook 组装 `{ event, value }` 载荷发布）。
