...
纯按钮组件：`variant`（形态）× `palette`（语义色板）× `size` 三轴。**无 loading、无 icon 机制**——IconButton/LoadingButton 等组合形态由组合式组件另行实现（内核纯粹性优先）。
...
├── types/index.ts                   # ButtonProps / ButtonRef / ButtonSize / ButtonVariant / ButtonPalette
├── variants/
│   ├── size.ts / variant.ts / palette.ts   # 各轴类名常量
│   └── index.ts                     # buttonVariants = cva（三轴 + 默认值）
├── styles/
│   ├── base.scss                    # 结构 + focus-visible + disabled 指针
│   ├── palette.scss                 # palette → 主题色组局部变量映射
...
stories 已迁 `apps/preview/src/button/`（variant/palette/size/state 四页，消费方视角 `import { Button } from '@colox/react'`）。
...
- 导出 `Button`、`ButtonProps`、`ButtonRef`、`ButtonSize`、`ButtonVariant`、`ButtonPalette`、`buttonVariants`、`ButtonVariants`。
