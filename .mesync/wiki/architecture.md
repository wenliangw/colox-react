...
- **ColoxTheme 运行时结构（@colox/theme/src/components/theme-context/）**：组件是自持单元——`index.tsx`（根组件，props 承载 theme/defaultTheme/palette）+ `context.ts`（ColoxThemeContext，直接提供 register/unregister，无独立注册 hook）+ `storage/`、`breakpoints/`（子组件各占独立文件夹，挂载即注册）+ `hooks/use-colox-theme.ts`（订阅）+ `stores/theme-store.ts`（模块级单例：三轴 + 跟随系统 + 持久化；浏览器 API 在 subscribe 提交阶段惰性接线，SSR 纯渲染零副作用）+ `utils/registry.ts`（子组件注册表折叠）+ `constants/theme.ts`（全部魔法值，类型 union 用 typeof 同源派生）+ `types/index.ts`。测试独立在 `test/` 按模块分目录（theme-context/stores/cli/config + utils/setup），跨模块引用 `@/` 别名、组件内部相对路径。`'system'` 在主题值域内（跟随系统；setTheme 手动值即脱离、'system' 即回归，isFollowSystem 由内部推导）；Storage 子组件显隐即开关（localStorage 键 colox:theme / colox:palette，恢复优先级 storage > props > default）；测试 jsdom 16 例 + CLI 11 例 + 身份回归 2 例（共 29，test/ 全量）。
