...
- **ColoxTheme 运行时结构（@colox/theme/src/ColoxTheme/）**：store.ts 模块级单例（theme/palette/breakpoint 三轴 + 跟随系统 + 持久化，浏览器 API 在 subscribe 提交阶段惰性接线，SSR 纯渲染零副作用）→ ColoxThemeContext.tsx（注册表 + useColoxTheme，useSyncExternalStore 订阅）→ parts.tsx（四 part 纯声明，layout effect 注册/注销）→ ColoxTheme.tsx（根：折叠注册表→施加 store，同类 last-write-wins）。`'system'` 在主题值域内（跟随系统；setTheme 手动值即脱离、'system' 即回归，isFollowSystem 由内部推导）；Storage part 显隐即开关（localStorage 键 colox:theme / colox:palette，恢复优先级 storage > part > default）；测试 jsdom 16 例 + CLI 11 例 + 身份回归 2 例。
