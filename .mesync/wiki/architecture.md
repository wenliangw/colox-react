...
- **聚合 CSS 入口 index.css**：`pnpm build` 末尾拼接产出 `dist/index.css`（style.css + palette/light/dark 三主题件的串联，同一份声明零复制），exports `./index.css`——使用方一行 `import '@colox/react/index.css'` 全量引入；颗粒入口保留为单一职责文件。拼接合法性：palette 与语义命名空间互斥 + 属性轴 `:root[...]` 特异性稳压基线，节序无关；CLI 编译的自定义主题/色板文件在其后加载即按源顺序覆盖同名声明。
