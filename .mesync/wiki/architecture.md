...
- **双包拆分（@colox/theme + @colox/react）**：`@colox/theme`（packages/theme）拥有样式与主题系统全部机器——Figma token 管线（meta/tokens/scripts/sd 三配置）、主题编译 CLI（colox bin）、JSON Schema、`config/theme.default.json` 标准配置模板、dist 主题件；将来 ColoxTheme 运行时与 ThemeBuilder（`builder` 子路径）也在此。`@colox/react` 只剩组件（代码/scss/stories/tests），构建时 `@import '@colox/theme/index.css'` 把主题级联打进 style.css（自包含单行引入，重声明同值幂等）；build 链「theme 先行」。组件只读 `var(--colox-*)`、不 import theme 代码——依赖是纯运行时 CSS 契约。标准配置模板做 CLI 身份回归测试（编译空配置 == 官方存量 palette 基线，锁死编译器与 SD 管线同步）。
