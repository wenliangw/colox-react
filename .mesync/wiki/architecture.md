...
- **ColoxTheme 运行时结构（@colox/theme/src/，@ 别名 = src）**：按能力目录分层——`context/`（`index.tsx` 根组件 + `parts.tsx` 四 part + `types/index.ts` 类型面）、`hooks/`（`use-colox-theme` 订阅 + `use-colox-theme-registry` 注册表）、`stores/theme-store.ts`（模块级单例：三轴 + 跟随系统 + 持久化；浏览器 API 在 subscribe 提交阶段惰性接线，SSR 纯渲染零副作用）、`utils/registry.ts`（注册表折叠）、`constants/theme.ts`（全部魔法值：attribute 名/存储键/媒体查询/词汇，类型 union 用 typeof 同源派生）。测试独立在 `test/` 按模块分目录（context/stores/cli/config + utils/setup），src 内引用一律 `@/` 别名（tsconfig paths + vite/vitest alias 三处齐配）。`'system'` 在主题值域内（跟随系统；setTheme 手动值即脱离、'system' 即回归，isFollowSystem 由内部推导）；Storage part 显隐即开关（localStorage 键 colox:theme / colox:palette，恢复优先级 storage > part > default）；测试 jsdom 16 例 + CLI 11 例 + 身份回归 2 例（共 29，test/ 全量）。
