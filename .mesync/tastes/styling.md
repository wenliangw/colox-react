...
- 不引入重型 css-in-js。
- **变体扩展性：现状维持 closed union + 逃生舱，css-in-js 仅作决策背景知识（用户已澄清非偏好）**：用户对两代 css-in-js 是「了解细节以便决策」，**非偏好，现阶段明确不考虑**。当前立场不变：内置变体 + 封闭 union（类型诚实优先），用户自定义样式走包装函数/className 逃生舱（未来可补组件级 token）。已调研的代价认知（备查用）：运行时 css-in-js 自由组合但背 SSR/运行时/双渲染税；零运行时 in-js（Panda 类）把注入搬构建期、要求消费方跑 codegen，与「预编译 npm 包」交付互斥（Chakra v3/Park UI 均转为 snippet 交付）。若未来真需要「自由组合生成变体 + 样式自动生效」，先厘清交付模型取舍再重开议题——不可因好奇调研误读成方向信号。
