...
- **微动效只消费 motion token，门控集中一处**：组件声明 transition 一律用 `--colox-motion-duration-*`（fast 100ms/normal 200ms/slow 300ms）+ `--colox-motion-easing-*`（out/in/in-out），禁止自造时长缓动；全局开/关由 theme 的 motion.css 门控层统一执行（`data-colox-motion` 轴 + `prefers-reduced-motion`），组件零分支、风格天然一致。装饰投影（shadow）走独立 boolean 轴，常态 shadow-md、hover 升 shadow-lg（首个微动效实例）。
