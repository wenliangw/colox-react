...
## 改名类重构：支出方与消费方同步，产物必抽检

- 全局改名（token 名、CSS 变量名、属性名）改完**定义侧**后，必须全仓搜**消费侧**引用一并替换——「生产者改完、消费者漏改」是 Button hover 全白故障的根因（sed 只替换了左值，右侧 `var(--colox-color-brand-solid--hover)` 引用名未同步）。
- **CSS 变量断裂的隐蔽性**：jsdom 测试、vite 构建、tsc 全都抓不到（`var()` 运行时解析，未定义变量静默判无效→样式回退）；唯一可靠验证是**抽检构建产物**（grep dist 确认新名存在、旧名计数为 0）。
- 改名类重构 checklist：① 定义侧改名 → ② 全仓消费侧同步替换（sed/grep 双向确认）→ ③ 重建产物抽检 → ④ 旧名全仓清零。
- 会话中对用户声明「已入库/已验证」的，必须是真实落盘/真实执行过的结论——口头声明不算数。
