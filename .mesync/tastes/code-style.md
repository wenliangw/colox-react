...
- 工具方法与类型**都要写注释**（内容短则单行 `/** … */`；多行走 `/**`／`*`／` */` 三段式，可带 `@default`）。
...
## JSDoc 书写：多行内容的闭合符必须独立成行

- **短内容允许单行**：`/** 一句话 */` 合法——内容确实只有一行时，不必为格式凑多行。
- **多行内容必须三段式**：`/**` 独占首行、`*` 逐行续写、` */` 独占末行；**禁止把 `*/` 附在最后一行文本尾部**（`/** text\n * more. */` 的附尾式）。tag（`@default` 等）单独成行，续行 `*` 后单空格。
- 来源：用户 2026-10 澄清——「并不是不允许单行注释，有些内容确实比较少，可以单行注释；注释不允许的是内容写了多行，但是结尾的 `*/` 符号却没有换行，这样很不美观」。此前记录的「所有 JSDoc 一律多行」是对规则的过度解读，已纠正。
