...
- **运行时产物扩展走契约形状**：token 延伸的运行时常量发射由 `runtime: {type, output}` 声明（`type` 为将来格式留位），能力归属编译期包而非运行时包——「哪些 token 值需要运行时使用」是编译期的知识。**output 是目录**：文件名是 builder 的内部决定（breakpoints.ts），用户不感知（运行时变量是组件库默认行为，未来新增运行时变量往同目录发射新文件，不破坏契约）。
