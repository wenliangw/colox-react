...
- gray is a dedicated neutral ramp (keeps pure white at 50, dark end deep
  enough for dark-mode surfaces).
...
def qa(hexv):
    """Return a QC string like 'white AA 4.9 / black AA 12.1'."""
    L = luminance(hexv)
    w = contrast(1.0, L)
    d = contrast(L, luminance("#1A1A1A"))
    parts = []
    parts.append("white %.1f%s" % (w, " AA" if w >= 4.5 else ""))
    parts.append("black %.1f%s" % (d, " AA" if d >= 4.5 else ""))
    return " / ".join(parts)

# ---- shared staircase (approved red template) ------------------------------

STAIRCASE = [
    # (lightness, saturation)  50 -> 900 — identical to the approved red staircase
...
# 800 ~= #383838, 900 ~= #1A1A1A (match the original gray dark end for dark surfaces)
...
lines = []
lines.append("# Colox color scale proposal (unified family staircase)\n")
lines.append("> Generated by `packages/components/scripts/generate_color_scales.py`\n")
lines.append("> Rule: chromatic scales share one lightness/saturation staircase (hue differs only); gray is a dedicated neutral ramp (50 keeps pure white).\n")

for name in ["indigo", "purple", "blue", "green", "orange", "red", "gray"]:
    scale = SCALES[name]
    ls = [luminance(v) for v in scale.values()]
    mono = "monotonic" if all(ls[i] > ls[i + 1] for i in range(len(ls) - 1)) else "non-monotonic"
    lines.append(f"\n## {name}\n")
...
for name in ["indigo", "purple", "blue", "green", "orange", "red", "gray"]:
    ...
    for step in STEPS:
        v = SCALES[name][step]
        L = luminance(v)
        flag = " non-monotonic" if prev and L >= prev else ""
